...
#Make multiple plots for different types of models
#PACKAGES:
import matplotlib
...
Mlist= [0.1*u.Msun,0.3*u.Msun,0.5*u.Msun,0.7*u.Msun,0.9*u.Msun,1.1*u.Msun]
angle=bestangle
for i in range(6):
	majorax=amp*float(majax1.strip())
...
# Set up a second Axis on top of pv plot, to show annotations (if necessary) ##?## invisible axes just to make annotations
	...
	F4.show_lines([np.array([velvalues.value+vc.to(u.m/u.s).value,rvalarcs.to(u.deg).value+halflength.to(u.deg).value])],color='cyan',linewidths=2.0,linestyles='-')
		#since we also have negative radii we als have to add
	F4.show_lines([np.array([velvalues.value+vc.to(u.m/u.s).value,-rvalarcs.to(u.deg).value+halflength.to(u.deg).value])],color='cyan',linewidths=2.0,linestyles='-')
	F4.add_label(0.15,0.8,str(M.value)+' Mo',relative=True,color='yellow',size='large')
	#MODIFY AXES:
	'''F4.refresh() 
